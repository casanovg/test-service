...
async def test_endpoint():
    """Simple test endpoint - returns a test message"""
    return {
...
@router.post("/echo", tags=["Test"])
async def echo_endpoint(data: Dict[str, Any]):
    """Echo endpoint - returns whatever you send"""
    return {"echo": data, "message": "Successfully echoed your data"}
...
async def get_examples(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
async def get_example(example_id: int, db: Session = Depends(get_db)):
    """Get example by ID"""
    example = db.query(models.ExampleModel).filter(models.ExampleModel.id == example_id).first()
...
async def update_example(
    example_id: int, example: schemas.ExampleUpdate, db: Session = Depends(get_db)
):
    """Update example"""
    db_example = db.query(models.ExampleModel).filter(models.ExampleModel.id == example_id).first()
